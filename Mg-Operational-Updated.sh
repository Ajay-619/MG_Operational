...
do
    ...
    if [ -n "${LATEST_RUN_ID}" ]; then
        ...
        RUN_DETAILS=$(curl -s -X GET \
                ...
                #| jq -r '.runid, .starting, .start, .end, .status, .profile.profileName, .profile.namespace')
        ...
        PROFILE_NAME=$(echo ${RUN_DETAILS} | jq -r '.profile.profileName')
        NAMESPACE=$(echo ${RUN_DETAILS} | jq -r '.profile.namespace')
...
#        RUN_DETAILS_JSON=$(jq -c -n --arg index "df_pipeline" --arg profileName "$PROFILE_NAME" --arg namespace "$NAMESPACE" --arg pipeline "$CDF_PIPELINE" --arg runId "$RUN_ID" --arg starting "$STARTING_TIME" --arg start "$START_TIME" --arg end "$END_TIME" --arg status "$STATUS" --argjson errorLogs "$ERROR_LOGS" '{index: $[index], profileName: $[profileName], namespace: $[namespace], pipeline: $[pipeline], runId: $[runId], starting: $[starting], start: $[start], end: $[end], status: $[status], errorLogs: $[errorLogs]}')
        RUN_DETAILS_JSON='{"index": "df_pipeline", "profileName": "'$PROFILE_NAME'", "namespace": "'$NAMESPACE'", "pipeline": "'$CDF_PIPELINE'", "runId": "'$RUN_ID'", "starting": "'$STARTING_TIME'", "start": "'$START_TIME'", "end": "'$END_TIME'", "status": "'$STATUS'", "errorLogs": "'$ERROR_LOGS'"}'
